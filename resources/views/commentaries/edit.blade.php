...
<form action="{{ route('commentaries.update', $commentary) }}" enctype="multipart/form-data" method="POST">
    {{ csrf_field() }}
    @method('PUT')
    <div class="form-group">
        ...
        <input class="form-control" id="fName" name="name" placeholder=" Name" required="required" value="{{ $commentary->name }}" type="text"/>
    </div>
    <div class="form-group">
        <label for="nacionality">
            * Nacionality
        </label>
        @include('form.country', ['name' => 'nacionality', 'value' => $commentary->nationality])
    </div>
    <div class="form-group">
        <label for="nacionality">
            * Program
    ...
    <div class="form-group">
        ...
        @include('form.date', ['name' => 'from', 'required' => false, 'disabled' => false])
    </div>
    <div class="form-group">
        <label id="from">
            * To
        </label>
        @include('form.date', ['name' => 'to', 'required' => false, 'disabled' => false])
    </div>
    <div class="form-group">
        <label id="comment">
            * Comment
        </label>
        <textarea class="form-control" id="comment" maxlength="500" name="comment" placeholder="Comment (Minimum 500 characters!)" required="required">{{ $commentary->commentary }}</textarea>
    </div>
